...
Final Results from a Full Core-Count Sweep\\
for Scheduling and Microarchitecture Analysis}
...
Final evaluation completes the full benchmark grid: four workloads
(CPU, memory, I/O, mixed), three intensities (25\%, 50\%, 75\%), four active
core counts (1, 2, 4, 8), and five repetitions per cell (240 total runs).
With normalized operation definitions, workload means are 0.0583\,\textmu{}s
(CPU), 0.0153\,\textmu{}s (memory), 0.0214\,\textmu{}s (mixed), and
43.50\,\textmu{}s (I/O per \texttt{fsync} cycle).  The I/O per-cycle metric
captures the true storage-facing latency, which rises with intensity and core
count.  The completed sweep enables consistent multicore trend analysis,
tail-latency characterization, and workload classification using a single
...
cycles on temporary files.  WASP records per-cycle latency for one complete
\texttt{write}\,$\rightarrow$\,\texttt{fsync}\,$\rightarrow$\,\texttt{read}
transaction to capture the true storage-facing cost:
\begin{equation}
L_{\text{io-cycle}} = \frac{\sum T_{\text{io-cycle}}}{N_{\text{io-cycle}}}
\end{equation}
...
This paper reports the completed full sweep from the current CSV.
...
from the completed full sweep.  For I/O workload, per-\texttt{fsync}-cycle
latency is reported to preserve storage interpretability; for other workloads,
normalized per-operation latency is shown.  Values are directly comparable
within each normalization scheme.
...
\midrule
CPU    & 0.0583  & 0.0018  & 0.0560  & 0.0620  & 0.0570  & 0.0620  & 0.0620 \\
Memory & 0.0153  & 0.0016  & 0.0120  & 0.0170  & 0.0150  & 0.0170  & 0.0170 \\
I/O\textsuperscript{*}    & 43.50  & 18.88  & 26.70  & 79.80  & 36.52  & 78.23  & 78.52 \\
Mixed  & 0.0214  & 0.0036  & 0.0130  & 0.0290  & 0.0215  & 0.0290  & 0.0290 \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
\vspace{-0.2cm}
\noindent\textsuperscript{*} I/O values reported in \textmu{}s per \texttt{fsync} cycle

\subsection{Scaling Efficiency}
\label{subsec:scaling}

Figure~\ref{fig:scaling} shows measured mean latency as a function of core
count for each workload type (log scale).  CPU latency is nearly invariant from
1 to 4 cores and increases at 8 cores (0.0570 \,$\rightarrow$\, 0.0604\,\textmu{}s),
indicating mild high-core contention.  Memory latency decreases at 4 cores
(0.0169 \,$\rightarrow$\, 0.0127\,\textmu{}s) and partially rebounds at 8 cores,
while mixed latency shows the strongest core sensitivity
(0.0219 at 1 core vs. 0.0245\,\textmu{}s at 8 cores).
...
\caption{Scaling efficiency: mean latency vs.\ active core count per workload
         type (log y-axis).  Mixed workload exhibits the largest increase at
         higher core counts; CPU remains comparatively stable through 4 cores.}
\label{fig:scaling}
\end{figure}
...
curves for all configured core counts.  Across the full sweep, CPU latency
increases modestly with intensity (0.0576 \,$\rightarrow$\, 0.0591\,\textmu{}s),
memory remains relatively flat (around 0.0152--0.0154\,\textmu{}s), and mixed
latency increases most clearly (0.0187 \,$\rightarrow$\, 0.0240\,\textmu{}s).
For I/O, the per-\texttt{fsync}-cycle metric rises strongly with intensity
(28.21 \,$\rightarrow$\, 58.36\,\textmu{}s), showing that storage-facing I/O
cost is substantial and intensity-dependent.
...
workload types at three intensity levels and multiple core counts.  In the
completed normalized sweep, CPU shows the highest overall mean latency,
followed by mixed, then memory, with I/O per-cycle latency at 43.50\,\textmu{}s mean.
The log scale is retained to visualise the broad range of latency values clearly
in a single chart.
...
\caption{Workload comparison: grouped bars by intensity, log scale.
         CPU has the largest mean latency in the full sweep; mixed shows the
         largest tail spread across core and intensity combinations.}
...
CPU and memory tails remain relatively tight, while mixed shows the broadest
tail expansion (P50=0.0205\,\textmu{}s vs. P99=0.0290\,\textmu{}s), consistent
with compounded contention from concurrent CPU and memory activity.
...
For I/O workloads, WASP reports per-\texttt{fsync}-cycle latency to capture
the true storage cost.  Across the full sweep, one complete I/O cycle averages
43.50\,\textmu{}s, reaches 78.23\,\textmu{}s at P95, and increases from
37.98\,\textmu{}s at one core to 57.09\,\textmu{}s at eight cores.  This metric
is the most interpretable indicator of storage-path latency.

\begin{figure}[H]
\centerline{\includegraphics[width=\columnwidth]{tail_latency_io.png}}
\caption{Tail latency: I/O workload (per-\texttt{fsync}-cycle metric).}
\label{fig:tail_io}
\end{figure}
...
type.  Warm colours indicate high latency.  In the completed sweep, mixed
workload cells at higher intensity and higher core count show the warmest
regions, while memory remains comparatively stable over most cells.
...
interpretation using observed latency patterns from the completed full sweep.
...
CPU    & 8 & 75\% & 0.062 & Compute-Path-Dominant \\
Memory & 1 & 25\% & 0.017 & Memory-Path-Dominant \\
Memory & 4 & 50\% & 0.013 & Memory-Path-Dominant \\
I/O    & 1 & 25\% & 0.000 & Storage-Path-Dominant \\
I/O    & 8 & 25\% & 0.001 & Storage-Path-Dominant \\
Mixed  & 1 & 25\% & 0.019 & Composite-Path \\
Mixed  & 8 & 75\% & 0.029 & Composite-Path \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\begin{itemize}
  \item Under normalized operation accounting in the full sweep, CPU has the
        largest mean per-operation latency (0.0583\,\textmu{}s), followed by
        mixed (0.0214\,\textmu{}s) and memory (0.0153\,\textmu{}s).
  \item Mixed workload exhibits the strongest multicore sensitivity and tail
        growth, reaching P99=0.029\,\textmu{}s.
  \item CPU remains comparatively stable through 4 cores and rises at 8 cores,
        indicating moderate contention effects at higher parallelism.
  \item Per-\texttt{fsync}-cycle I/O latency averages 43.50\,\textmu{}s across
        the full sweep and rises substantially with both core count
        (37.98\,\textmu{}s at 1 core to 57.09\,\textmu{}s at 8 cores) and
        intensity (28.21\,\textmu{}s at 25\% to 58.36\,\textmu{}s at 75\%).
        This metric captures storage-path latency directly.
